...
void hist_average(const String& img)
{
    Mat src,gray;
    //读取图象
    src = imread(img);
    ...
    long sum = gray.rows*gray.cols;
    vector<double> hist(256);
    for (int i = 0; i < 256; ++i) {
        if (i>0){ hist[i] = nums[i]*1.0/sum + hist[i-1];}
        else hist[i] = nums[i]*1.0/sum;
    }
    for(int i=0;i<output.rows;i++)
    {
        ...
        for (int j=0;j<output.cols;j++) {
            double a = p[j];
            int num = int(hist[a]*255);
            if (a>255 or a<0){ printf("wrong");}
            p[j] = num;
        }
    }
    for(int i=0;i<255;i++) nums[i] = 0;
    for (int i = 0; i < output.rows; i++)
    {
        uchar *p = output.ptr<uchar>(i);
        for (int j = 0; j < output.cols; j++)
